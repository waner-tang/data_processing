...
def process_line(line):
    ...
    if len(columns) == 6:
        ...
        # 使用Decimal来保持精度并重新排列
        new_line = f"{Decimal(x)} {Decimal(y)} {Decimal(z)} {nx} {ny} {nz} \n"
        return new_line
    else:
        # 如果不是6列数据，直接返回原始行
        print("errors!!")
...
if __name__ == "__main__":
    input_directory = "data/data-test"  # 输入文件夹路径
    output_directory = "data/data-test2"  # 输出文件夹路径
